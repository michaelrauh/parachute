...
pub fn single_process(registry: &Registry) -> Registry {
    ...
    let r = fold_up_by_origin_repeatedly(r, new_squares);
    r
}

fn fold_up_by_origin_repeatedly(r: Registry, new_squares: Vec<Ortho>) -> Registry {
    std::iter::successors(Some((r, new_squares)), |(current_registry, current_squares)| {
...
fn find_additional_squares(
    ...
    check_back.iter().flat_map(|(left, center, right)| {
        match (left, center, right) {
            ...
            (Item::Square(l), Item::Pair(_), Item::Square(r)) => handle_connection(combined_book, l, r),
...
fn handle_connection(registry: &Registry, l: &&Ortho, r: &&Ortho) -> Vec<Ortho> {
    ...
    potential_corresponding_axes.into_iter().flat_map(|correspondence| {
...
fn all_other_connections_work(
    ...
    l.contents()
        .iter()
        .all(|other_connection| l.connection_works(other_connection.to_string(), registry, correspondence, &r))
}

fn find_potential_correspondences(registry: &Registry, l: &&Ortho, r: &&Ortho) -> Vec<Vec<(String, String)>> {
    let left_axes = l.get_hop();
    let right_axes = r.get_hop();
    let potentials: Vec<(String, String)> = iproduct!(left_axes, right_axes).filter(|(left_try, right_try)| {
        registry.contains_line_with(left_try, &right_try)
    }).collect();
...
fn combobulate_axes(potentials: Vec<(String, String)>) -> Vec<Vec<(String, String)>> {
    let num_axes = potentials.iter().map(|(left, _)| left).collect::<HashSet<_>>().len();
...
fn sufficient_axes_to_cover(potentials: &[(String, String)], l: &Ortho) -> bool {
    ...
    let (left, right): (HashSet<_>, HashSet<_>) = potentials.iter()
